...
const statusOptions = [
  ...
  { value: "validated", label: INVOICE_STATUS_LABELS.validated },
...
export const InvoiceUpdateForm: React.FC<InvoiceUpdateFormProps> = ({
  className,
  updateInvoice,
  ...
  useEffect(() => {
    if (invoice) {
      const typeTemplates = templates.filter(
        (t) => t.type === invoice.type && t.is_active
      );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (invoice?.type === "facture" && !formData.due_date) {
  ...
  const handleChange = (field: keyof UpdateInvoiceRequest, value: any) => {
  ...
  const updateItem = (
    index: number,
    field: keyof NonNullable<UpdateInvoiceRequest["items"]>[0],
    value: any
  ) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const taxRate = formData.tax_rate || invoice?.tax_rate || 20;
  ...
  return (
    <form onSubmit={handleSubmit} className={cn("space-y-6", className)}>
      ...
      <div className="space-y-2">
        <Label>Type</Label>
        <Input
          value={invoice.type === "devis" ? "Devis" : "Facture"}
          disabled
          className="bg-muted"
        />
      </div>
      ...
      <div className="space-y-2">
        ...
        <Select
          ...
          disabled={isUpdatePending || !formData.client_id}
        >
          <SelectTrigger>
            ...
            >
              ...
              <div className="col-span-2 flex items-end">
                <Button
                  ...
                  disabled={
                    isUpdatePending || (formData.items?.length || 0) === 1
                  }
